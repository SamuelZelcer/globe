...
func (h *updateHandler) UpdatePassword(ctx echo.Context) error {
	...
	tokens, err := h.service.UpdatePassword(ctx.Request().Context(), request, splitAuthHeader[1])
	if err != nil {
		return ctx.JSON(http.StatusBadRequest, map[string]string{"error": err.Error()})
	}
	if tokens.RefreshToken == "" {
		return ctx.JSON(http.StatusOK, "Verification code was sent to your email")
	}
